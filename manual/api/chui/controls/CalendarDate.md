[chui](../../index.md) › [chui.controls](index.md) › CalendarDate
...
- `CuiException` — `Time.currentDateTime()`（见同版本 SDL API 参考中的 `sdl.system.Time`）读取系统实时钟或把时间戳换算为本地日历字段失败时；CangHui 不捕获或改写该异常。
