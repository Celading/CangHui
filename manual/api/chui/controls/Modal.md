[chui](../../index.md) › [chui.controls](index.md) › Modal

# Modal

`chui.controls` 包中的 public class

模态对话框：`presented` 为真时把 `body` 子树布局在一块对视口居中的面板里，经浮层浮于整棵界面树之上并压暗背景，事件一律不再落到背景树。对话框承载的是真实组件子树——按钮、文本框乃至会再弹浮层的控件都按平常方式工作；内容未处理的 Escape 或按下背景可关闭。
...
关闭途径：内容完全展开后，Escape 先交给内容处理；例如输入框正在预编辑时，第一次 Escape 只取消组合文本，下一次才关闭弹窗。内容未消费的 Escape 或按下背景把 `presented` 写回 `false`、移除浮层并调用 `onDismiss`；应用代码直接把 `presented` 置 `false`（如“取消”按钮）同样关闭，但不触发 `onDismiss`。展开动画期间仍由弹窗直接处理 Escape，不向隐藏内容派发。
