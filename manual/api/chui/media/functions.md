[chui](../../index.md) › [chui.media](index.md) › 函数

# 函数 — chui.media

`chui.media` 的包级函数：图像纹理缓存的刷新入口。缓存按文件路径键控、按渲染器归属，以下操作作用于所有活跃渲染器，只能在共同 UI owner 线程使用。失败记录也计入条目上限，缓存命中期间不重复读盘；预算与诊断见[图片内存与帧池](../../../reference/image-memory.md)。
