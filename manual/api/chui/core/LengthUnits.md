[chui](../../index.md) › [chui.core](index.md) › LengthUnits

# LengthUnits

`chui.core` 包中的 public interface

为数值字面量提供 `.px`/`.vp`/`.fp` 长度后缀的接口。`Int64` 与 `Float64` 的[扩展](extensions.md)实现了它，使 `100.px`、`24.vp`、`1.5.vp`、`15.fp` 直接产出 [`Length`](Length.md)；导入本接口（`chui` 根包已再导出）即启用后缀。
