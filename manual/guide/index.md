...
第一次使用，请沿“从这里开始”完成首窗口和设置表单；已有项目可从“按任务查找”直接进入具体结果。由 Agent 操作时，先完成[Agent 首次工作流](../getting-started/agent-first-workflow.md)和[Agent UI 评审规则](how-to/agent-ui-review.md)，不要跳过依赖边界与 cuic 基线。页面中的 `role=complete` 是可放进空项目的完整程序；`role=patch` 明确依赖所链接的基页；concept 的片段用于比较和追踪，不应误当成独立程序。
...
[焦点、事件与浮层](concepts/focus-events-and-overlays.md) → [Modal 与 Toast](how-to/modal-and-toast.md) → [键盘与焦点](how-to/keyboard-and-focus.md) → [文本与菜单](how-to/text-editing-and-menus.md) → [可编辑文本装饰](how-to/decorate-editable-text.md)。约 75 分钟；结束时鼠标、菜单和快捷键复用同一动作，并能把应用生成的令牌与诊断安全绘制在可编辑文本上。
...
只想让属性变化自动带出动画？先看[属性驱动动画](how-to/animate-properties.md)。需要
选择卡、设置行或不同入口结构，可直接从 [CangHuiKit](../reference/canghui-kit.md) 的示例开始。
...
| 审查按钮高度、三槽信息行、图标和文本密度 | [Agent UI 评审](how-to/agent-ui-review.md) | 窄/常规/宽视口与亮/暗主题均有 ASCII 和必要像素证据 |
...
| 给可编辑文本添加语法、搜索或诊断样式 | [可编辑文本装饰](how-to/decorate-editable-text.md) | 修订过期时旧样式安全消失，新快照到达后恢复；编辑、IME 与撤销不变 |
