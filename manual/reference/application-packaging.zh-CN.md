...
CUIC 打包会在整个生成期间持有输出目录旁的 `.cuic-lease`，并在取得锁后重新检查
输出是否为空。需要构建的路由还会从构建开始到复制、硬化、审计及回执写完一直
持有目标目录锁。遇到其他 CUIC 任务持锁会立即拒绝，不等待或抢占；构建失败和
后续异常都会释放本次取得的锁，但不会删除失败产物或其他进程的锁。
这只协调使用相同目标/输出路径的 CUIC 任务，不阻止外部工具直接修改文件。
