...
struct chui_ak_tree *chui_ak_tree_new(const char *window_label) {
    ...
    tree->nodes[0] = (struct entry){1, 0, root};
    tree->count = 1;
    tree->focus = 1;
    index_entry(tree, 0);
    return tree;
...
bool chui_ak_tree_add(struct chui_ak_tree *tree,
    ...
    tree->nodes[index] = (struct entry){id, parent_id, node};
...
struct accesskit_tree_update *chui_ak_tree_finish(struct chui_ak_tree *tree) {
    ...
    struct accesskit_tree_update *update = accesskit_tree_update_with_capacity_and_focus(tree->count, tree->focus);
