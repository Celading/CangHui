...
| 移动桥接 | iOS 原生表面生命周期切片、Android 表面启动边界、阶段化 package receipt、外部签名器关联、平台 owner 签名包证据与 kMode 回放 | CangHui 不执行签名器或验证器；`installable` 只表示可尝试安装，实际安装、启动、真机回放与消费者验收仍按平台分别推进 |
