...
统一导入入口：本包自身不定义类型或函数，而是重新导出 sdl 模块和全部 CangHui 子包。sdl 提供几何、颜色、渲染、图像、窗口、输入与桌面系统能力；CangHui 子包提供组件、布局、控件、文本、媒体和桌面应用。应用只需 `import chui.*` 即可使用这些 API。

## 再导出

以下符号定义于本模块各子包，在此再导出；详情见各定义包的文档页。

**来自 [`chui.core`](core/index.md)**
...
| [`ClipPolicy`](core/Surface.md#clippolicy) | 组合表面的子内容是否裁到当前 shape 外框。 |
| [`ControlRole`](core/InteractionSurface.md#controlrole) | `InteractionSurface` 对外发布的按钮、链接、勾选、开关或自定义语义角色。 |
...
| [`InteractionSurface`](core/InteractionSurface.md) | 让任意装饰内容拥有一个焦点、动作、状态与语义所有者的交互组合原语。 |
...
| [`MaterialProvider`](core/Surface.md#materialprovider-与-materials) | 按主题与 `SurfaceState` 解析完整 surface 外观的只读接口。 |
| [`Materials`](core/Surface.md#materialprovider-与-materials) | 内置的固定色、主题 panel 与主操作 material provider。 |
...
| [`Shape`](core/Surface.md#shape) | `Surface` 与 `InteractionSurface` 共用的矩形或圆角矩形几何。 |
...
| [`Surface`](core/Surface.md) | 只负责 material、shape、clip 与前景环境，不夺取 child 交互身份的装饰组合容器。 |
| [`SurfaceMaterial`](core/Surface.md#surfacematerial) | 一次解析得到的表面样式、主/辅前景与 ink 色。 |
...
**来自 [`chui.desktop`](desktop/index.md)**
...
以下符号定义于 **sdl 模块**（本文档范围之外），由伞包再导出以便应用单一导入。每个分组标题都链接到同版本 SDL API 的包索引；在这里按符号确认可直接导入的名称，再从对应包索引进入类型页查看精确声明和成员。本节不计入 CangHui 手册的覆盖率。
...
| `ImageFileFormat` | `SdlSurface` 能读写的图像文件格式。 |
...
| 包 | 说明 |
|---|---|
| [`chui.core`](core/index.md) | UI 核心：组件与修饰器、布局容器、基础控件、状态、动画、主题、每帧上下文、浮层和焦点。 |
| [`chui.controls`](controls/index.md) | 成品控件库：选择与数值输入、下拉与菜单、导航、数据展示、模态、Toast、折叠面板、分栏与可重排列表。 |
| [`chui.text`](text/index.md) | 文本编辑控件：单行 TextField、多行 TextArea、带建议的 ComboBox 与共享编辑模型 TextEditState。 |
| [`chui.media`](media/index.md) | 图像与自绘：文件图像视图 ImageView（进程级纹理缓存）与自由绘制表面 CanvasWidget。 |
| [`chui.desktop`](desktop/index.md) | 桌面应用对象：DesktopApp 拥有窗口与渲染循环，驱动构建-布局-绘制-事件分发。 |
