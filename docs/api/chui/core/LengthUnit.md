[chui](../../index.md) › [chui.core](index.md) › LengthUnit
...
CangHui 在逻辑坐标空间中布局，三种单位描述数值如何映射进这个空间：`Vp` 就是逻辑坐标单位本身，解析即原值；`Px` 是物理设备像素，解析时除以窗口显示缩放；`Fp` 是虚拟像素再乘以用户字体缩放，专供文字尺寸跟随系统无障碍设置。
